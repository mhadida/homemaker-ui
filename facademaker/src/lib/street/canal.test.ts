...
describe("canalGroundMaskTriangles", () => {
  it("covers the full outer-bank width with two terrain-height triangles", () => {
    const mask = canalGroundMaskTriangles(
      [[0, 0], [40, 0]],
      14,
      { slope: 0, azimuth: 0 },
    );
    expect(mask).toHaveLength(6);
    expect(mask.every((p) => p[1] === CANAL_MASK_LIFT)).toBe(true);
    const zs = mask.map((p) => p[2]);
    expect(Math.max(...zs)).toBeCloseTo(10.5);
    expect(Math.min(...zs)).toBeCloseTo(-10.5);
  });

  it("drapes the mask over sloping terrain", () => {
    const ground = { slope: 0.1, azimuth: 0 };
    const mask = canalGroundMaskTriangles([[0, 0], [0, 40]], 14, ground);
    expect(mask).toHaveLength(6);
    for (const [x, y, z] of mask) {
      expect(y).toBeCloseTo(groundHeightAt(x, z, ground) + CANAL_MASK_LIFT);
    }
  });
});
