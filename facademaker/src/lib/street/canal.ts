...
export const CANAL_MASK_LIFT = 0.05;  // above terrain solely for stencil projection
...
/** Terrain-height ribbon covering the whole canal cut. On a real heightfield
 * the ground cannot use ShapeGeometry holes, so CanalMesh renders these
 * triangles into the stencil buffer before the ground. The ground then skips
 * matching pixels and the recessed blue water/quays remain visible. */
export function canalGroundMaskTriangles(
  centreline: Vec2[],
  width: number,
  ground: Ground,
): Vec3[] {
  ...
  const { bank } = canalOffsets(centreline, width);
  const point = ([x, z]: Vec2): Vec3 => [
    x,
    groundHeightAt(x, z, ground) + CANAL_MASK_LIFT,
    z,
  ];
  const out: Vec3[] = [];
  for (let i = 0; i < bank.left.length - 1; i++) {
    const l0 = point(bank.left[i]);
    const r0 = point(bank.right[i]);
    const r1 = point(bank.right[i + 1]);
    const l1 = point(bank.left[i + 1]);
