...
export default function CanalMesh({
  ...
  const geos = useMemo(() => {
    ...
    const bedPos: number[] = [];
    const maskPos = ground.hf
      ? canalGroundMaskTriangles(cl, w, ground).flat()
      : [];
    ...
    return {
      ...
      walk: mk(walkPos),
      bed: mk(bedPos),
      mask: maskPos.length > 0 ? mk(maskPos) : null,
    };
  }, [street, ground]);
  ...
  return (
    ...
    >
      {/* A real heightfield is one displaced grid and cannot carry the exact
      ...
      <mesh geometry={geos.water} renderOrder={1}>
