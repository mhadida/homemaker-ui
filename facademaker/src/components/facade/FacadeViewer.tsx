...
function PlanPane({
  ...
  // first fit the user controls the camera via MapControls. Imported layers
  // arrive independently, so their empty/non-empty transitions each earn one
  // fit; ordinary edits within an existing layer do not.
  const fitStage = `${blocks.length > 0}:${streetNetwork.streets.length > 0}:${contextGeometry !== null}:${ground.hf !== undefined}`;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const fit = useMemo(() => sceneBounds, [fitStage, size.w, size.h]);
  const zoom = fitOrthoZoom(size.w, size.h, fit.width, fit.depth);
...
function PerspectivePane({
  ...
  const [walkExitTarget, setWalkExitTarget] = useState<
    [number, number, number] | null
  >(null);
  // A context/street/terrain-only scene needs a true overview pose. Snapshot
  // once per imported-layer arrival so later inspector edits and hides never
  // wrest camera control back from the user.
  const fitStage = `${streetNetwork.streets.length > 0}:${contextGeometry !== null}:${ground.hf !== undefined}`;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const overviewFit = useMemo(() => perspectiveFitFor(sceneBounds), [fitStage]);
  const overviewActive = blocks.length === 0 && sceneBounds.hasContent;
  const orbitTarget = walkExitTarget ??
    (overviewActive ? overviewFit.target : ([0, 4, 0] as [number, number, number]));
  const cameraPosition = overviewActive
    ? overviewFit.position
    : ([6, 5, 14] as [number, number, number]);
  return (
    <>
      <SceneContents
        blocks={blocks}
      ...
      <PerspectiveCamera
        ...
        fov={40}
        near={0.1}
        far={Math.max(PERSPECTIVE_FAR, overviewFit.far)}
      />
      {walk ? (
        <WalkControls
          ground={ground}
        ...
        <OrbitControls
          ...
          maxDistance={Math.max(
            ORBIT_MAX_DISTANCE,
            overviewActive ? overviewFit.maxDistance : 0,
          )}
  ...
  const worldH = block
    ? mode === "overview"
      ...
      : lotLayout
        ...
        : 12
    : sceneBounds.height;
  const targetY = block ? worldH / 2 : sceneBounds.cy;
  const midX = block
    ? mode === "overview"
  ...
  const position = useMemo<[number, number, number]>(
    () =>
      block
        ...
        : [sceneBounds.cx, sceneBounds.cy, sceneBounds.maxZ + ELEVATION_DISTANCE],
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [block, mid, normal3[0], normal3[2], sceneBounds.maxZ],
  );
  const farPlane = block
    ? 400
    : Math.max(400, sceneBounds.depth + ELEVATION_DISTANCE * 2 + 100);
  const camRef = useRef<THREE.OrthographicCamera>(null);
  useEffect(() => {
    const cam = camRef.current;
    if (!cam) return;
...
export default function FacadeViewer({
  ...
  const contextGeometry = useMemo(
    () =>
      shownContextBuildings.length > 0
        ? buildContextGeometry(shownContextBuildings, ground)
        : null,
  ...
  // The classic WebGLRenderer is the production default while WebGPU quad-view
  // rendering remains under repair. `?webgpu` opts into native WebGPU,
  // `?webgl2` exercises WebGPURenderer's fallback backend, and `?stats` shows
  // an FPS panel for A/B measurement.
  const useWebGPU = isWebGPUPath();
  const showStats =
    typeof window !== "undefined" &&
    new URLSearchParams(window.location.search).has("stats");
  const heavyScene =
    shownContextBuildings.length > 500 || streetNetwork.streets.length > 100;
  ...
  return (
    ...
    >
      ...
      <Canvas
        key={canvasKey}
        shadows
        frameloop={walkMode || showStats ? "always" : "demand"}
          ...
          useWebGPU
            ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
              (async (props: any) => {
                ...
                await renderer.init();
                // R3F accepts an async renderer factory, but its initial
                // setSize can run before that promise resolves. WebGPU then
                // keeps the default 300×150 depth attachment while the color
                // attachment follows the real canvas, invalidating every
                // quad-view render pass. Seed the resolved renderer with the
                // live CSS size; R3F's normal resize observer takes over from
                // here.
