...
import { footprintBase } from "@/lib/geo/buildings";
...
const EMPTY_BUILDINGS: ContextBuilding[] = [];
...
export function buildContextGeometry(
  ...
  return { geo, faceBuilding, buildings };
}
...
export default function ContextBuildings({
  ...
  const shown = model?.buildings ?? EMPTY_BUILDINGS;
  ...
  const hoverGeo = useMemo(() => {
    ...
    return b ? buildContextGeometry([b], ground).geo : null;
  }, [litId, shown, ground]);
  ...
  const idAt = (e: ThreeEvent<PointerEvent | MouseEvent>): string | undefined =>
    typeof e.faceIndex === "number" ? model.faceBuilding[e.faceIndex] : undefined;
